...
// Test Account
// ============================================
const TEST_EMAIL = 'test@tamilaistream.com';
const TEST_PASSWORD = 'Test@123';
const TEST_NAME = 'Test User';

function seedTestAccount() {
    const existing = findUserByEmail(TEST_EMAIL);
    if (!existing) {
        createUser(TEST_EMAIL, TEST_PASSWORD, TEST_NAME);
...
async function quickTestLogin() {
    ...
    try {
        ...
        let user = validateCredentials(TEST_EMAIL, TEST_PASSWORD);
        if (!user) {
            user = createUser(TEST_EMAIL, TEST_PASSWORD, TEST_NAME);
        }
        if (!user) {
        ...
        // Set testSession so builder.html recognizes the test login
        ...
        // Also set main website session
        try {
            localStorage.setItem('tamilAIStream_user', JSON.stringify({
                uid: user.uid || 'test-local',
        ...
        DOM.successTitle.textContent = 'Welcome Test User!';
        DOM.successMessage.textContent = 'Loading Test Mode Builder...';
        DOM.successOverlay.classList.add('visible');

        setTimeout(() => {
            window.location.href = 'builder.html?mode=test&auto=1';
        }, 1500);
        ...
        btn.classList.remove('loading');
        btn.disabled = false;
        showToast(error.message || 'Test login failed', 'error');
    }
}

async function openTestBuilderFromLogin() {
    const btn = document.getElementById('openTestBuilderBtn');
    if (!btn) return;

    btn.classList.add('loading');
    btn.disabled = true;
    const originalHTML = btn.innerHTML;
    btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Opening Test Builder...';
    ...
    try {
        ...
        if (!user) {
            throw new Error('Failed to prepare test session');
        }
        ...
        showToast('Opening Test Mode Builder...', 'success');
        ...
        btn.classList.remove('loading');
        btn.disabled = false;
        showToast(error.message || 'Failed to open Test Builder', 'error');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('testLoginBtn')?.addEventListener('click', quickTestLogin);
    document.getElementById('openTestBuilderBtn')?.addEventListener('click', openTestBuilderFromLogin);
