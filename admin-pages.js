...
(function() {
    ...
    function showProgress(pct, text) {
        ...
        $('progressText').textContent = text || 'Saving...';
    }
    function hideProgress() {
        $('progressFill').style.width = '100%';
        $('progressText').textContent = 'Done!';
        setTimeout(() => { $('progress-bar').classList.remove('show'); $('progressFill').style.width = '0'; }, 800);
    }
    ...
    function navigateTo(page) {
        ...
        if (page === 'visual-editor') setTimeout(() => { try { AdminEditor.detectSections(); AdminEditor.detectElements(); } catch(e) {} }, 100);
    }
    ...
    function renderDashboard() {
        ...
        const hist = (PublishManager.getChangesHistory ? PublishManager.getChangesHistory() : []).slice(0, 5);
        $('dashHistory').innerHTML = hist.length ? hist.map(e =>
    ...
    function renderSongs(body) {
        ...
        function filter(q) {
            ...
            $('songsTable').innerHTML = f.length ? f.map(s =>
        ...
        filter('');
        $('songSearch').addEventListener('input', e => filter(e.target.value.toLowerCase()));
    }

    window.editSong = function(idx) {
        const songs = DataStore.getSongs();
        const s = idx >= 0 ? songs[idx] : { id: 'song_' + Date.now(), title: '', artist: '', album: '', thumbnail: '', audioUrl: '', duration: 0 };
        $('mContentEditTitle').innerHTML = '<i class="fas fa-' + (idx < 0 ? 'plus' : 'pen') + '"></i> ' + (idx < 0 ? 'Add Song' : 'Edit Song');
        $('mContentEditBody').innerHTML = formField('Title','text','title',s.title) + formField('Artist','text','artist',s.artist) + formField('Album','text','album',s.album) + formField('Thumbnail URL','url','thumbnail',s.thumbnail) + formField('Audio URL','url','audioUrl',s.audioUrl) + formField('Duration (sec)','number','duration',s.duration);
        $('mContentEditSave').onclick = () => {
            const data = {};
    ...
    function renderStations(body) {
        ...
        function filter(q) {
            ...
            $('stationsTable').innerHTML = f.length ? f.map(s =>
        ...
        filter('');
        $('stationSearch').addEventListener('input', e => filter(e.target.value.toLowerCase()));
    }
    ...
    window.editStation = function(idx) {
        ...
        $('mContentEditTitle').innerHTML = '<i class="fas fa-pen"></i> ' + (idx < 0 ? 'Add Station' : 'Edit Station');
        $('mContentEditBody').innerHTML = formField('Name','text','name',s.name) + formField('Frequency','text','freq',s.freq) + formField('Stream URL','url','streamUrl',s.streamUrl) + formField('Genre','text','genre',s.genre) + formField('City','text','city',s.city) + formField('Thumbnail','url','thumbnail',s.thumbnail);
        $('mContentEditSave').onclick = () => {
            const data = {};
    ...
    window.editAlbum = function(idx) {
        ...
        $('mContentEditTitle').innerHTML = '<i class="fas fa-pen"></i> ' + (idx < 0 ? 'Add Album' : 'Edit Album');
        $('mContentEditBody').innerHTML = formField('Title','text','title',a.title) + formField('Artist','text','artist',a.artist) + formField('Thumbnail','url','thumbnail',a.thumbnail);
        $('mContentEditSave').onclick = () => {
    ...
    function renderSongsCollections(body) {
        ...
        ['left','right'].forEach(side => {
            $(side === 'left' ? 'scLeft' : 'scRight').innerHTML = (data[side] || []).map((item, i) =>
    ...
    window.editMC = function(idx) {
        ...
        $('mContentEditTitle').innerHTML = '<i class="fas fa-pen"></i> Collection';
        $('mContentEditBody').innerHTML = formField('Name','text','name',c.name);
        $('mContentEditSave').onclick = () => {
    ...
    window.editMovie = function(idx) {
        ...
        $('mContentEditTitle').innerHTML = '<i class="fas fa-pen"></i> Movie';
        $('mContentEditBody').innerHTML = formField('Title','text','title',m.title) + formField('Year','text','year',m.year) + formField('Thumbnail','url','thumbnail',m.thumbnail);
        $('mContentEditSave').onclick = () => {
    ...
    window.editYearly = function(idx) {
        ...
        $('mContentEditTitle').innerHTML = '<i class="fas fa-pen"></i> Year';
        $('mContentEditBody').innerHTML = formField('Year','text','year',y.year) + formField('Title','text','title',y.title) + formField('Thumbnail','url','thumbnail',y.thumbnail);
        $('mContentEditSave').onclick = () => {
    ...
    window.editUpcoming = function(idx) {
        ...
        $('mContentEditTitle').innerHTML = '<i class="fas fa-pen"></i> Upcoming';
        $('mContentEditBody').innerHTML = formField('Title','text','title',u.title) + formField('Date','text','date',u.date) + formField('Thumbnail','url','thumbnail',u.thumbnail);
        $('mContentEditSave').onclick = () => {
    ...
    function renderSectionSettings() {
        ...
        $('sectionsList').innerHTML = Object.entries(settings).map(([id, sec]) =>
        ...
        $('sectionsList').querySelectorAll('input, select').forEach(el => {
            el.addEventListener('change', () => {
                const settings = DataStore.getSectionSettings();
                const sid = el.dataset.sid;
                if (!settings[sid]) return;
                settings[sid][el.dataset.field] = el.type === 'checkbox' ? el.checked : el.value;
    ...
    function renderDesign() {
        ...
        body.querySelectorAll('[data-dfield]').forEach(el => {
            el.addEventListener('change', () => {
                const settings = DataStore.getSiteSettings();
                if (!settings.styles) settings.styles = {};
                const parts = el.dataset.dfield.split('.');
                let obj = settings.styles;
                for (let i = 0; i < parts.length - 1; i++) { if (!obj[parts[i]]) obj[parts[i]] = {}; obj = obj[parts[i]]; }
                obj[parts[parts.length - 1]] = el.type === 'number' ? Number(el.value) : el.value;
    ...
    function renderHeaderSettings() {
        ...
        $('headerBody').innerHTML = '<h3 style="font-size:12px;font-weight:600;margin-bottom:10px;">Logo</h3>' +
        ...
        $('headerBody').querySelectorAll('[data-nidx]').forEach(el => {
            el.addEventListener('change', () => {
                const nav = DataStore.getNavigation();
                if (!nav.items) nav.items = items;
                nav.items[parseInt(el.dataset.nidx)][el.dataset.nfield] = el.type === 'checkbox' ? el.checked : el.value;
    ...
    function renderLogoSettings() {
        ...
        $('logoBody').innerHTML = formField('Logo URL','url','logo',logo.logo||'') +
        ...
        $('logoBody').querySelectorAll('[data-field]').forEach(el => {
            el.addEventListener('change', () => {
                const logo = DataStore.getLogoSettings();
                logo[el.dataset.field] = el.type === 'checkbox' ? el.checked : (el.type === 'number' ? Number(el.value) : el.value);
    ...
    function renderSplashSettings() {
        ...
        $('splashBody').innerHTML = '<label class="fg-row" style="gap:6px;margin-bottom:10px;"><label class="toggle"><input type="checkbox" data-field="enabled"' + (cfg.enabled ? ' checked' : '') + '><span class="slider"></span></label><span style="font-size:11px;">Enabled on ' + _splashPlatform + '</span></label>' +
        ...
        $('splashBody').querySelectorAll('[data-field]').forEach(el => {
            el.addEventListener('change', () => {
                const entrance = DataStore.getEntranceLogo();
                if (!entrance[_splashPlatform]) entrance[_splashPlatform] = {};
                entrance[_splashPlatform][el.dataset.field] = el.type === 'checkbox' ? el.checked : (el.type === 'number' ? Number(el.value) : el.value);
    ...
    function renderPlayerSettings() {
        ...
        $('playerBody').innerHTML = '<h3 style="font-size:12px;font-weight:600;margin-bottom:10px;">Mini Player</h3>' +
        ...
        $('playerBody').querySelectorAll('[data-pfield]').forEach(el => {
            el.addEventListener('change', () => {
                const prefs = DataStore.getPlayerPrefs();
                const parts = el.dataset.pfield.split('.');
                let obj = prefs;
                for (let i = 0; i < parts.length - 1; i++) { if (!obj[parts[i]]) obj[parts[i]] = {}; obj = obj[parts[i]]; }
                obj[parts[parts.length - 1]] = el.type === 'checkbox' ? el.checked : (el.type === 'number' ? Number(el.value) : el.value);
    ...
    function renderPlatformSettings() {
        ...
        const sections = DataStore.getSectionSettings();
        $('platformBody').innerHTML = '<p style="font-size:11px;color:var(--text3);margin-bottom:14px;">Per-platform card sizes for <strong>' + _platformTab + '</strong> only.</p><div class="section-grid">' +
        ...
        $('platformBody').querySelectorAll('[data-psid]').forEach(el => {
            el.addEventListener('change', () => {
                const settings = DataStore.getSectionSettings();
                const sid = el.dataset.psid;
                if (!settings[sid]) return;
                ...
                settings[sid].responsive[_platformTab][el.dataset.pfield] = Number(el.value);
                DataStore.setSectionSettings(settings);
            });
        });
    }
    ...
    function renderSectionList() {
        ...
        $('sectionList').innerHTML = sections.map(sec => {
        ...
        $('sectionList').querySelectorAll('.lp-item').forEach(el => {
            el.addEventListener('click', e => { if (e.target.closest('.lp-act')) return; AdminEditor.selectSection(el.dataset.sid); AdminEditor.setElementMode(false); renderSectionList(); renderPropertyPanel(); updateElementModeUI(); });
        });
        $('sectionList').querySelectorAll('.lp-act').forEach(btn => {
    ...
    function renderLayerList() {
        ...
        $('layerList').innerHTML = tree.map(n => renderNode(n, 0)).join('');
        $('layerList').querySelectorAll('.lp-item').forEach(el => {
            el.addEventListener('click', e => { if (e.target.closest('.lp-act')) return; AdminEditor.setElementMode(true); AdminEditor.selectElement(el.dataset.eid); renderLayerList(); renderPropertyPanel(); updateElementModeUI(); });
        });
        $('layerList').querySelectorAll('.lp-act').forEach(btn => {
    ...
    $('tabSections').addEventListener('click', () => { $('tabSections').classList.add('active'); $('tabLayers').classList.remove('active'); $('sectionList').style.display = ''; $('layerList').style.display = 'none'; });
    $('tabLayers').addEventListener('click', () => { $('tabLayers').classList.add('active'); $('tabSections').classList.remove('active'); $('layerList').style.display = ''; $('sectionList').style.display = 'none'; AdminEditor.detectElements(); renderLayerList(); });
    ...
    function renderPropertyPanel() {
        ...
        body.querySelectorAll('.rp-input, .rp-select, .rp-input-color, .rp-range').forEach(input => {
            const handler = () => { AdminEditor.setOverride(sid, input.dataset.prop, input.value); if (input.type === 'range') { const span = input.parentElement.querySelector('span'); if (span) span.textContent = input.value; } };
    ...
    function updateUndoRedo() { $('btnUndo').disabled = !AdminEditor.canUndo(); $('btnRedo').disabled = !AdminEditor.canRedo(); }
    ...
    document.querySelectorAll('[data-device]').forEach(btn => {
        btn.addEventListener('click', () => { document.querySelectorAll('[data-device]').forEach(b => b.classList.remove('active')); btn.classList.add('active'); AdminEditor.setDevice(btn.dataset.device); });
    });
    ...
    $('btnPreview').addEventListener('click', () => {
        ...
        if (_currentPage === 'visual-editor') {
            ...
            $('center-area').style.left = _previewMode ? '0' : '';
            $('center-area').style.right = _previewMode ? '0' : '';
        }
    });

    async function refreshStatus() { try { await PublishManager.refreshStatus(); $('tbStatus').textContent = 'All changes live'; } catch (e) {} }

    $('btnHistory').addEventListener('click', () => {
        openModal('mHistory');
        const entries = PublishManager.getChangesHistory ? PublishManager.getChangesHistory() : [];
        $('mHistoryBody').innerHTML = entries.length ? entries.slice(0, 80).map(e =>
            '<div class="h-entry"><div class="h-icon h-icon-sync"><i class="fas fa-pen"></i></div><div class="h-info"><div class="h-title">' + (e.type || 'edit') + '</div><div class="h-detail">' + (e.details || '') + '</div><div class="h-meta"><span>' + new Date(e.timestamp).toLocaleString() + '</span></div></div></div>'
        ).join('') : '<div class="empty-state"><p>No changes yet</p></div>';
    });
    ...
    $('btnVersions').addEventListener('click', async () => {
        ...
        try {
            ...
            $('mVersionsBody').innerHTML = versions.length ? versions.map(v =>
        ...
        } catch (e) { $('mVersionsBody').innerHTML = '<div class="empty-state"><p>Error: ' + e.message + '</p></div>'; }
    });
    $('mSaveSnap').addEventListener('click', async () => { try { await PublishManager.saveVersionSnapshot('Manual snapshot'); toast('Snapshot saved', 'ok'); $('btnVersions').click(); } catch (e) { toast('Failed', 'err'); } });
    ...
    $('btnRefreshSections').addEventListener('click', () => { AdminEditor.detectSections(); AdminEditor.detectElements(); renderSectionList(); renderLayerList(); toast('Refreshed', 'info'); });

    function updateElementModeUI() { const on = AdminEditor.isElementMode(); $('pillElementMode').classList.toggle('on', on); $('pillElementMode').classList.toggle('off', !on); $('pillElementModeText').textContent = on ? 'Element Mode ON' : 'Element Mode'; }
    $('pillElementMode').addEventListener('click', () => { AdminEditor.setElementMode(!AdminEditor.isElementMode()); updateElementModeUI(); toast(AdminEditor.isElementMode() ? 'Element mode on' : 'Section mode', 'info'); });
    AdminEditor.on('element-mode-changed', updateElementModeUI);

    $('btnCopy').addEventListener('click', () => { AdminEditor.copyProperties(); toast('Copied', 'info'); });
    $('btnPaste').addEventListener('click', () => { AdminEditor.pasteProperties(); toast('Pasted', 'ok'); renderPropertyPanel(); });
    document.addEventListener('keydown', e => {
        if ((e.ctrlKey || e.metaKey) && e.key === 'c' && !window.getSelection().toString()) AdminEditor.copyProperties();
        if ((e.ctrlKey || e.metaKey) && e.key === 'v') { AdminEditor.pasteProperties(); renderPropertyPanel(); }
    });

    $('btnLock').addEventListener('click', () => { const id = AdminEditor.getSelectedElementId() || AdminEditor.getSelectedSectionId(); if (!id) { toast('Select first', 'err'); return; } AdminEditor.toggleLock(id); toast(AdminEditor.isLocked(id) ? 'Locked' : 'Unlocked', 'info'); renderLayerList(); });

    function renderGlobalSettings() {
        const gs = AdminEditor.getGlobalSettings();
        const body = $('globalPanelBody');
        const vars = [
        ...
        body.querySelectorAll('.rp-input, .fg-color').forEach(input => {
            input.addEventListener('change', () => {
                const gs2 = AdminEditor.getGlobalSettings();
                if (!gs2[input.dataset.gcat]) gs2[input.dataset.gcat] = {};
                gs2[input.dataset.gcat][input.dataset.gkey] = input.value;
    ...
    $('btnGlobal').addEventListener('click', () => {
        ...
        if (panel.style.display === 'none') { panel.style.display = ''; rightBody.style.display = 'none'; if (rpTabs) rpTabs.style.display = 'none'; $('rpTitle').innerHTML = '<i class="fas fa-globe"></i> Global Settings'; renderGlobalSettings(); }
        else { panel.style.display = 'none'; rightBody.style.display = ''; if (rpTabs) rpTabs.style.display = ''; $('rpTitle').innerHTML = '<i class="fas fa-sliders"></i> Properties'; renderPropertyPanel(); }
    });
