...
function updateNowPlayingBar(title, station, showBar) {
    ...
    // Slim Now Playing Bar
    const slimBar = document.getElementById('slimNowPlaying');
    const snpCurrent = document.getElementById('snpCurrent');
    ...
    if (slimBar && snpCurrent) {
        ...
        if (idx >= 0 && idx < q.length - 1 && snpNext) {
            const next = q[idx + 1];
            snpNext.textContent = 'Next: ' + (next.title || '—');
        } else if (snpNext) {
            snpNext.textContent = 'Next: —';
        }
    }
}
...
// Wire up Slim Now Playing bar close button
function initNowPlayingBar() {
    const snpClose = document.getElementById('snpClose');
    if (snpClose) {
        snpClose.addEventListener('click', () => {
            const slimBar = document.getElementById('slimNowPlaying');
            if (slimBar) slimBar.style.display = 'none';
        });
    }
        ...
        allSections.forEach(section => {
            ...
            if (sectionOrder.includes(type)) {
                section.style.display = '';
                       ...
                       type === 'ai-songs-collections' || type === 'ur-auto-slider') {
